...
/*
Пример создан сайтом: 
http://blogprogram.ru/klass-dlya-raboty-s-ftp-s-pomoshhyu-php-chast-3/
*/
...
Class FTPClient {
	...
	public function logMessage($message = false) { if($message != false) $this->messageArray[] = $message; else return $this->messageArray; }

	//сединение с FTP
	public function connect ($connection_data, $isPassive = true)
		{		
		$server 		= $connection_data['host'];
		$ftpUser 		= $connection_data['user'];
		$ftpPassword 	= $connection_data['pass'];
		...
		//проверка соединения
		if ((!$this->connectionId) || (!$loginResult)) { $this->logMessage('Ошибка подключения по FTP!'); return false;}
	...
	if(@ftp_rmdir($this->connectionId, $directory)) { $this->logMessage('Директория '.$directory.'" удалена'); }
	else { $this->logMessage('Не удалось удалить директорию "'.$directory.'"'); }
	}
	...
	*/
	...
	if (!$upload) { $this->logMessage('Не удалось загрузить файл!'); }
	else { $this->logMessage('Загружен "'.$fileFrom.'" как "'.$fileTo.'"'); }
	}
	...
	*/
	...
	if (!$upload) { $this->logMessage('Не удалось скачать файл!'); }
	else { $this->logMessage('Скачан "'.$fileFrom.'" как "'.$fileTo.'"'); }
	}

	//удалить файл на сервере
	public function delFile($directory) {
		if(@ftp_delete($this->connectionId, $directory)){ $this->logMessage('Файл '.$directory.'" удален'); }
		else { $this->logMessage('Не удалось удалить файл "'.$directory.'"'); }
	}
