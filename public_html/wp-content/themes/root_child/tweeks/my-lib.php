...
/*		Список функций:
*
* 4.	Функция получающая список фаилов лажащих в папке
* 5.	Функция получающая время последней модификации фаила
* 6.	Функция получает из БД массив разрешенных фаилов со статусом ON
* 7.	Функция принимает список handle фаилов и путь до папки где они лежат
* 8.	Функция удаляет из текста коментарии и пробелы и возвращает min контент
* 9.	Функция проверяет существование папки по указанному пути и создает её если она не создана
* 10.	Функция принимает имя фаила и вырезает у него раcширение, возвращает handle
* 11.	Функция убирает расширения у массива фаилов
* 12.	Функция просмотра функция привязанных к хуку (по имени хука)
* 13.	Функция конвертации многомерного нумерованного массива в строку
* 14.   Код ищет в каком фаиле на сервере находится функция (ПРОТЕСТИРОВАТЬ!!!)
*/
...
function last_mod($Files_url){	

	date_default_timezone_set("Europe/Moscow");	
	
	/* Получаем время*/
	foreach ($Files_url as $item){	
	
		//print_arr(filemtime($item));
		//print_arr($maxTime);
		
		$maxTime = max($maxTime, filemtime($item));	
	}
	
	/*Получаем имя фаила*/
	foreach ($Files_url as $item){
		if(filemtime($item) === $maxTime){ 
						
			$file = pathinfo($item)['filename'];						
			$path = pathinfo($item)['dirname'];
			}		
	}
	
	
	$maxTime = date ("d F Y  H:i:s", $maxTime);
	$lastmodified = array( 
								$file=> $maxTime, 
								'path'=>$path
								);		
...
/* #6
*	Функция получает из БД массив разрешенных фаилов со статусом ON
*	
*/

function ON_styles_list(){
	
	global $wpdb;
	//Запрашиваем БД у каких handle есть status ON
		$Db_status_on = $wpdb->get_results( 'SELECT handle FROM css_disable WHERE status="ON"', ARRAY_A);
				
	//Записываем handle-ы в массив $NOT_enqueue
		foreach ($Db_status_on as $item){$ON_styles_list[] = $item['handle']; }	
	
		
	

	return $ON_styles_list;
}



/* #7
* Записываем весь контент из списка разрешенных фаилов ($ON_styles_list) в переменную $content
* Функция принимает список handle фаилов и путь до папки где они лежат
*/	

function get_on_file_contents ($ON_styles_list, $dir){
	
	
	foreach ($ON_styles_list as $file){
			
			
		$abs_dir = $dir.$file.'.css';			
		$contents .= file_get_contents($abs_dir);	//.= Означает добавить данные в переменную (не перезаписать, а добавить) 
...
/* #9
* Создаем папку по указанному пути ($dir_folder) в которой будем хранить сжатый фаил. 
*/
function creat_folder($dir_folder) {
			
		//Создаем папку под сжатые фаилы если не создана//	
		if (!file_exists($dir_folder)) //Если папка еще не создана, создаём её. Если создана идём дальше.
			{						
			mkdir($dir_folder);
			}
...
/* #12 Просмотр функций привязанных к хуку 
...
/* #13 Функция конвертации многомерного нумерованного массива в строку
*  Принимает на вход многомерный нумерованный массив и возвращает в строке все его значения.
*/
...
/*
код ищет где в каком фаиле находится функция
$file_function = new ReflectionFunction('имя_функции');
