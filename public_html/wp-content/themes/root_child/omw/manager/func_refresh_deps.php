<?php
/* 	функция перезаписывающая CSS и JS фаилы а так же их min версии с распределением по страницам */
...
function refreash_deps($type, $current_state_structure){
	...
	/* СБОР ДАННЫХ О ТОМ ЧТО НАДО БУДЕТ ПОДКЛЮЧИТЬ */
		...
		/* Собираем наименования объектов для которых могут быть опции */
	...
	//print_arr($cfg_settings, 'настройки из конфига');
		...
		foreach($found_libs_res['found'] as $name => $src){												
			//te($name, 'found_libs_res NAME' );
			//te($src, 'found_libs_res SRC');		
	...
	/* ОПРЕДЕЛЕНИЕ КАКИЕ MIN ФАИЛЫ НУЖНО ПЕРЕЗАПИСЫВАТЬ */
		...
		*/			
		...
		/* МОДУЛЬ ПЕРЕЗАПИСИ MIN ФАИЛОВ */
			//if(!$page_options){$page_options = array();}
	...
	if($type == 'js'){
		...
		//print_arr($data , 'Дата идущая к Worker-у');
